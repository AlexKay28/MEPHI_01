...
class TestPolygon:
    ...
    # набор тестов для модифицированной программы =====
    # отрезок не вылезает из внутренней окружностии
    def test_mod0(self):
        t = Segment(R2Point(-1.0, 0.0), R2Point(1.0, 0.0))
        assert t.g() == 0

    # отезок делит окружность ровно на пополам
    def test_mod1(self):
        t = Segment(R2Point(0.0, 4.0), R2Point(0.0, -4.0))
        assert t.g() == approx(4.0)

    # прямоугольный треугольник с гипотенузой поностью
    # попадающей в пространоство кольца
    def test_mod2(self):
        t = Segment(R2Point(1.5, 0.0), R2Point(0.0, 1.5))
        t = t.add(R2Point(0.0, 1.0))
        assert t.g() == approx(3.3146955)

    # квадрат с двумя сторонами попадающеими в область кольца
    def test_mod3(self):
        t = Segment(R2Point(1.2, 0.0), R2Point(0.0, 1.2))
        t = t.add(R2Point(0.0, 0.0))
        t = t.add(R2Point(1.2, 1.2))
        assert t.g() == approx(1.2*2+2*0.2)

    # многоугольник с сторанами частично попадающими под область кольца
    def test_mod4(self):
        t = Segment(R2Point(-1.0, -1.0), R2Point(1.0, -1.0))
        t = t.add(R2Point(-1.0, 1.0))
        t = t.add(R2Point(1.0, 1.0))
        t = t.add(R2Point(0.0, 2.0))
        assert t.g() == approx(3*2+2*sqrt(2))

    def test_mod5(self):
        t = Segment(R2Point(2, 0), R2Point(0, 2))
        assert t.g() == approx(2 * sqrt(8))

    def test_mod6(self):
        t = Segment(R2Point(2, 0), R2Point(0, 2))
        t = t.add(R2Point(-2, 0))
        assert t.g() == approx(2 * sqrt(8) + 2)

    def test_mod7(self):
        t = Segment(R2Point(2, 0), R2Point(0, 2))
        t = t.add(R2Point(-2, 0))
        t = t.add(R2Point(0, -2))
        assert t.g() == approx(4 * sqrt(8))

    def test_mod8(self):
        t = Segment(R2Point(2, 0), R2Point(0, 2))
        t = t.add(R2Point(-2, 4))
        t = t.add(R2Point(2, 4))
        assert t.g() == approx(2.828427)
